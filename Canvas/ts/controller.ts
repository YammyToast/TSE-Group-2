...
export class Controller {
  ...
  handleHoverAnimations(_ctry: Country) {
    ...
    if (_ctry.hover == true && _ctry.active == false) {
      ...
      if (!_ctry.animationMap.has("hover")) {
        ...
        // for 255ms.
        let animation = new HoverAnimation(
          _ctry,
          "hover",
          255,
          startFill,
  ...
  viewChange(_view: number) {
    ...
    this.manager.handleYearSelection(
      (document.getElementById("yearslider") as any).value
    );
  }
  /**
  ...
  positionLabels(): void {
    ...
    for (let [key, label] of this.labelList.entries()) {
      ...
      label.content.css({
        top:
          this.canvasHeightTranslation +
          label.offsetY * this.canvasScaleFactorY,
        left:
          this.canvasWidthTranslation + label.offsetX * this.canvasScaleFactorX,
      });
    }
  }
  /**
  ...
  renderSelectBarLabels() {
    ...
    for (let key = 0; key < Object.entries(ViewTypes).length; key++) {
      ...
      let element: JQuery<HTMLElement> = createSelectItems(
        Object.entries(ViewTypes)[key][0],
        COLOURSLIGHT
      );
   ...
   */
  renderYearAverages(
    _yearAverages: DataYearAveragesInstance,
    _dataMinValue: number,
    _dataMaxValue: number,
    ...
    try {
      ...
      let colourGradient: RGB = {
        r:
          (_colourScheme.gradientDark.r - _colourScheme.gradientLight.r) /
          range,
        g:
          (_colourScheme.gradientDark.g - _colourScheme.gradientLight.g) /
          range,
        b:
          (_colourScheme.gradientDark.b - _colourScheme.gradientLight.b) /
          range,
      };
      let data: any = {};
  ...
  setLabelValues(_yearAverages: DataYearAveragesInstance) {
  ...
  constructor(
    _objects: {
      ctryList: Map<string, Country>;
      staticObjList: Map<string, ContourObject>;
    },
